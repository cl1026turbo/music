...
router.get("/",(req,res)=>{
  ...
  if(lid!==undefined){
    var sql1=`select * from shop_laptop where lid=?`;
    pool.query(sql1,[lid],(err,result)=>{
      if(err) console.log(err);
      output.product=result[0];
      var family_id=output.product["family_id"];
      var sql2=`select spec,lid from shop_laptop where family_id=?`;
      pool.query(sql2,[family_id],(err,result)=>{
        if(err) console.log(err);
        output.specs=result;
        var sql3=`select * from shop_laptop_pic where laptop_id=?`
